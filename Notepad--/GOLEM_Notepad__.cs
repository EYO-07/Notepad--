...
"Use a comment like this on top of document to override the lexer by file extension."
{Notepad--L:.cpp}
...
//			}
    ...
    private void SBR_load_modules() {
        ...
        bool b_has_any = false;
        if ( is_file( join(get_exec_dir(),"modules\\ghcmd.exe") ) ) {
            b_has_any = true;
    ...
    private void SBR_load_module_ghcmd() {
        ...
        if (  confirmation_dialog(
            "Load Global Hotkeys Module",
            "Did you want to load ghcmd.exe (global hotkeys) module? This module requires administrative privileges."
        ) ) {
